...
from keyboards.inline.user.callbacks import free_lessons_cb, free_less_cat_cb, free_subcat_cb
...
def view_free_subcategories_ikb(items, current_page, all_pages, category_id):
    btn = InlineKeyboardMarkup(row_width=1)
    for subcategory in items:
        btn.add(
            InlineKeyboardButton(
                text=subcategory['name'],
                callback_data=free_subcat_cb.new(
    ...
    btn.add(
        InlineKeyboardButton(
            text="⬅️ Ortga", callback_data=free_subcat_cb.new(
...
def view_free_lessons_ikb(items, current_page, all_pages, subcategory_id):
    ...
    btn.row(
        InlineKeyboardButton(
            ...
            callback_data=free_lessons_cb.new(
                action="prev", value=subcategory_id, c_pg=current_page
            )
        ),
        InlineKeyboardButton(
            text=f"{current_page}/{all_pages}",
        ...
        InlineKeyboardButton(
            text="▶️",
            callback_data=free_lessons_cb.new(
                action="next", value=subcategory_id, c_pg=current_page
            )
        )
    )
    btn.add(
        InlineKeyboardButton(
            text="⬅️ Ortga", callback_data=free_lessons_cb.new(
                action="back", value=subcategory_id, c_pg="1"
            )
        )
    )
    return btn
...
def content_back_ikb(subcategory_id, current_page):
    btn = InlineKeyboardMarkup(row_width=1)
    btn.add(
        InlineKeyboardButton(
            text="⬅️ Ortga", callback_data=free_lessons_cb.new(
                action="content_back", value=subcategory_id, c_pg=current_page
            )
        )
    )
    return btn
